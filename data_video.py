...
#         for root, dirnames, filenames in os.walk(data_dir):# 加载数据集路径
...
#         if ori_vlen / actual_fps * self.fps > self.max_num_frames:#截取视频帧???
...
#         first_frm = first_frm.permute(2,0,1)
        ...
        for root, dirnames, filenames in os.walk(data_dir):# 加载数据集路径
            for filename in filenames:
                if filename.endswith(".mp4"):
                    ...
                    # word_prompt_path = video_path.replace(".mp4",".txt").replace("videos","word_prompts")
                    # if os.path.exists(word_prompt_path):
                    #     word_prompt = open(word_prompt_path, "r").read().splitlines()[0]
                    # else:
                    #     word_prompt = ""
                    # self.word_prompts.append(word_prompt)
                    self.word_prompts.append("dog")
                    ...
                    if not os.path.exists(ref_image_path):
                        ref_image_path = ""
                    self.ref_images.append(ref_image_path)
