...
def make_dataset_full(path, split, args, create_dataset_function, 
    ...
    if (is_train_data and args.iterable_dataset) or (not is_train_data and args.iterable_dataset_eval): # cannot indexed 检查是否为可迭代数据集
...
def make_loaders(args, create_dataset_function, collate_fn=None):
    ...
    make_dataset = partial(make_dataset_full, 
                        create_dataset_function=create_dataset_function, batch_from_same_dataset=args.batch_from_same_dataset)# 将一个函数转换为另一个函数，可以对原函数的部分参数进行固定，从而产生一个新的函数
