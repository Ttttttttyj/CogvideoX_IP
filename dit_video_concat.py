...
class ImagePatchEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        super().__init__()
        self.proj = nn.Conv2d(in_channels, hidden_size, kernel_size=patch_size, stride=patch_size, bias=bias) #(16,1920,(2,2),(2,2))
        if text_hidden_size is not None:
            self.text_proj = nn.Linear(text_hidden_size, hidden_size) #(4096,1920)
        else:
            self.text_proj = None
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        B, T = images.shape[:2]
        emb = images.view(-1, *images.shape[2:]) # (b*t,c,h,w)
        emb = self.proj(emb)  # ((b t),d,h/2,w/2)
        emb = emb.view(B, T, *emb.shape[1:])
        emb = emb.flatten(3).transpose(2, 3)  # (b,t,n,d)=(2,13,1350,1920)
        emb = rearrange(emb, "b t n d -> b (t n) d")

        if self.text_proj is not None:
            text_emb = self.text_proj(kwargs["encoder_outputs"])
            emb = torch.cat((text_emb, emb), dim=1)  # (b,n_t+t*n_i,d)=(2,226+13*1350,1920)
...
class Basic3DPositionEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        self.width = width # 45
        self.text_length = text_length #226
        self.compressed_num_frames = compressed_num_frames #13
        self.spatial_length = height * width
