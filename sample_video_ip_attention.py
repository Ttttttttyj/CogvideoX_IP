...
def load_from_file(p, processor,rank=0, world_size=1):
    ...
    with open(prompt_path, "r") as fin:
        ...
        for l in fin:
            ...
            word_prompt_path = prompt_path.replace("prompt.txt","word_prompt.txt")
            with open(word_prompt_path,"r") as file:
                word_prompts = file.readlines()
...
def get_batch(keys, value_dict, N: Union[List, ListConfig], T=None, device="cuda"):
    ...
    for key in keys: #['ref_image', 'txt']
        if key == "txt":
            batch["txt"] = np.repeat([value_dict["prompt"]], repeats=math.prod(N)).reshape(N).tolist()
            batch_uc["txt"] = np.repeat([value_dict["negative_prompt"]], repeats=math.prod(N)).reshape(N).tolist()
        elif key == "ref_image":
            batch[key] = value_dict[key]
            batch_uc[key] = value_dict[key]
        else:
            batch[key] = value_dict[key]
    if T is not None:
...
def sampling_main(args, model_cls):
    ...
    num_samples = [1]
    force_uc_zero_embeddings = ["txt","ref_image"]
    device = model.device
    with torch.no_grad():
        for text, cnt,word_prompt,ref_img in tqdm(data_iter):
            model.to(device)
            if args.image2video:
...
if __name__ == "__main__":
    ...
    # data = load_from_file("./inference_test")
    # for text, cnt,word_prompt,ref_img in data:
    #     print(text,cnt,word_prompt,ref_img.shape)